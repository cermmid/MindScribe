...
def init_db() -> None:
    with _conn() as c:
        ...
        for col, decl in USAGE_COLUMNS.items():
        ...
        cur = c.execute(
            """INSERT INTO visits
               (created_at, audio_path, pipeline, raw_transcript, ai_note_original_json,
                status, doctor_id, prompt_tokens, output_tokens, total_tokens, estimated_cost_usd)
               VALUES (?, ?, ?, ?, ?, 'draft', ?, ?, ?, ?, ?)""",
            (
...
def list_visits() -> list[dict[str, Any]]:
    with _conn() as c:
        rows = c.execute(
            """SELECT id, created_at, status, pipeline, doctor_id,

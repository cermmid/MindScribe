...
class TestOfficialTitleWins:
    def test_replaces_model_description_with_who_title(self, fake_who):
        ...
        result = _verify(
            [ICDCode(code="QE80", description="Problemy związane ze snem", confidence=0.7)],
            klasyfikacja="ICD-11",
        )
    ...
    def test_no_note_when_model_description_matches(self, fake_who):
        ...
        result = _verify(
            [ICDCode(code="6B00", description="Generalised anxiety disorder", confidence=0.9)],
            klasyfikacja="ICD-11",
        )
        assert result[0].zweryfikowany is True
...
class TestCodeFromDiagnosisName:
    def test_finds_code_when_model_left_it_empty(self, fake_who):
        ...
        result = _verify(
            [ICDCode(code="", description="Zaburzenie lękowe uogólnione", confidence=0.8)],
            klasyfikacja="ICD-11",
        )
    ...
    def test_replaces_wrong_code_found_by_name(self, fake_who):
        ...
        result = _verify(
            [ICDCode(code="6A70", description="Zaburzenie lękowe uogólnione", confidence=0.8)],
            klasyfikacja="ICD-11",
        )
        assert result[0].code == "6B00"
        assert result[0].zweryfikowany is True
...
class TestUnverifiable:
    def test_marks_unverified_when_nothing_matches(self, fake_who):
        ...
        result = _verify(
            [ICDCode(code="ZZ99", description="Coś wymyślonego", confidence=0.9)],
            klasyfikacja="ICD-11",
        )
    ...
    def test_keeps_model_proposal_visible_when_unverified(self, fake_who):
        fake_who()
        result = _verify([ICDCode(code="ZZ99", description="Coś", confidence=0.5)])
    ...
    def test_api_failure_never_breaks_generation(self, fake_who):
        ...
        result = _verify(
            [
                ICDCode(code="6B00", description="Lęk uogólniony", confidence=0.9),
                ICDCode(code="6A70", description="Epizod depresyjny", confidence=0.8),
            ],
            klasyfikacja="ICD-11",
        )
        assert len(result) == 2
    ...
    def test_api_failure_is_not_retried_for_every_code(self, fake_who):
        ...
        _verify([ICDCode(code=f"X{i}", description="x", confidence=0.5) for i in range(5)])
...
class TestIcd10Path:
    def test_uses_code_lookup_for_icd10(self, fake_who):
        ...
        result = _verify(
            [ICDCode(code="F41.1", description="Lęk uogólniony", confidence=0.9)],
            klasyfikacja="ICD-10",
        )
        assert result[0].zweryfikowany is True
